...
export default function Header({ visible = true }) {
  ...
  // Cierra el menú móvil si se oculta el header
  useEffect(() => {
      ...
      {visible && (
        <motion.header
          ...
          className={`fixed top-0 inset-x-0 z-50 transition-all duration-500 ${
            scrolled
              ? 'bg-graphite-700/75 backdrop-blur-xl border-b border-hairline-d'
              : 'bg-transparent'
          }`}
        >
          <div className="container-px flex items-center justify-between h-16 sm:h-20">
